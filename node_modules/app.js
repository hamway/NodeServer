...
exports.tmpl = new just({root: this.home + '/view', useCache: false, ext: '.html'});
...
exports.getStatus = function (type) {
    ...
    if (type == "json") {
        ...
        this.tmpl.render('stats', {title: 'Server Stats'}, function (err, html) {
            api.res.writeHead(200, {'Content-Type': 'text/html'});
            api.res.write(html, 'utf-8');
        })
    }
    if (type != undefined) {
        this.sendClient(res);
...
exports.sendClient = function (text, type) {
    if (type == undefined) type = 'text/plain';
    if (text == undefined) {
        api.res.writeHead(503, { "Content-Type": type });
...
function objectToString(obj, level) {
    ...
    for (var key in obj) {
        ...
        if ((typeof value) != 'object') {
            ...
            res += key + ':\n'
            res += objectToString(value, level);
        }
    }
    return res;
}